...
def images_download(search_word, images_num):
    ...
    for search_word in search_words:        # 단어 설정
        ...
        all_imgs = soup.find_all("img", class_="YQ4gaf")       # 모든 img 선택
        imgs = [img for img in all_imgs if "zr758c" not in img.get('class', [])]
        ...
        for img in imgs:
            try:
                ...
                if width <= 1 or height <= 1:
                    print("Skipping small image:", src)
                    continue
...
search_words = ["croissant", "toast", "baguette", "iceamericano", "powerade"]
images_num = 120
